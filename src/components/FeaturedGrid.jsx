 ...
 * FeaturedGrid.jsx
 *
 * Renderiza os cards públicos na Home.
 * Lê da coleção "featured" onde visible == true, ordenado por createdAt desc.
 * Usa it.imageUrlCard || it.imageUrl.
 */
...
export default function FeaturedGrid() {
  ...
  return (
    <section className="max-w-6xl mx-auto p-4">
        ...
        {items.map((it) => (
          <article key={it.id} className="border border-neutral-800 rounded-xl overflow-hidden">
            ...
            <div className="p-3 grid gap-1">
              ...
              {it.description && (
                <p className="text-sm text-neutral-300 line-clamp-2">{it.description}</p>
              )}
      ...
      {items.length === 0 && (
        <div className="text-sm text-neutral-400 mt-3">Nenhum destaque disponível no momento.</div>
      )}
    </section>
  );
}
