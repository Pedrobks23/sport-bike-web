    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Histórico de Serviços</h2>
          <button
        ...
        {loading ? (
          ...
          <p className="text-gray-600">Nenhum serviço encontrado para este cliente.</p>
        ) : (
              ...
              <div key={ordem.id} className="border rounded-lg p-4 bg-gray-50">
                <div className="flex justify-between mb-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium 
                    ${ordem.status === 'Concluído' ? 'bg-green-100 text-green-800' : 
                    ordem.status === 'Em andamento' ? 'bg-blue-100 text-blue-800' : 
                ...
                <p className="text-gray-600 mb-1">
                  Data: {ordem.dataCriacao ? new Date(ordem.dataCriacao).toLocaleDateString() : 'N/A'}
                </p>
                  ...
                  <div className="mt-2">
                    ...
                    <ul className="list-disc list-inside">
                      {ordem.bicicletas.map((bike, index) => (
                        <li key={index} className="text-gray-600">{bike}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {ordem.totalBikes && (
                  <p className="text-gray-600">Total de bicicletas: {ordem.totalBikes}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-[#333]">Gerenciar Clientes</h1>
          </div>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {customers.filter(customer => 
            ...
            <div key={customer.id} className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              <h3 className="text-lg font-bold text-[#333] mb-2">{customer.nome}</h3>
              <p className="text-gray-600 mb-1">Telefone: {customer.telefone}</p>
              <p className="text-gray-600 mb-4">Email: {customer.email}</p>
              <p className="text-gray-600 mb-4">Endereço: {customer.endereco || '-'}</p>
              
              <div className="flex space-x-2">
                <button
                    onClick={() => handleViewServiceHistory(customer.id, customer.telefone)}
